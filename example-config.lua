...
local wayland_probe_cmd = [[sh -lc 'bin="${EVILWM_BIN:-evilwm}"; probe="$(dirname "$bin")/evilwm-probe-client"; if [ ! -x "$probe" ]; then printf "evilwm probe client not found: %s\n" "$probe" >&2; exit 1; fi; exec "$probe" xdg-window --title "evilwm probe wayland"']]
local panel_probe_cmd = [[sh -lc 'bin="${EVILWM_BIN:-evilwm}"; probe="$(dirname "$bin")/evilwm-probe-client"; if [ ! -x "$probe" ]; then printf "evilwm probe client not found: %s\n" "$probe" >&2; exit 1; fi; exec "$probe" layer-panel --namespace evilwm-public-baseline-panel']]
local x11_test_cmd = [[sh -lc 'bin="${EVILWM_BIN:-evilwm}"; probe="$(dirname "$bin")/evilwm-probe-client"; if [ -x "$probe" ]; then exec "$probe" x11-window --title "evilwm probe x11"; elif command -v xterm >/dev/null 2>&1; then exec xterm; elif command -v xeyes >/dev/null 2>&1; then exec xeyes; elif command -v xclock >/dev/null 2>&1; then exec xclock; elif command -v xcalc >/dev/null 2>&1; then exec xcalc; else printf "No simple X11 test app found (tried: evilwm-probe-client xterm xeyes xclock xcalc)\n" >&2; exit 1; fi']]
local screenshot_cmd = [[sh -lc 'bin="${EVILWM_BIN:-evilwm}"; sock="${EVILWM_IPC_SOCKET:-}"; if [ -z "$sock" ]; then printf "EVILWM_IPC_SOCKET is not set\n" >&2; exit 1; fi; dir="${XDG_PICTURES_DIR:-$HOME/Pictures}/evilwm"; mkdir -p "$dir"; out="$dir/$(date +%Y-%m-%d_%H-%M-%S).ppm"; "$bin" --ipc-socket "$sock" --ipc-command screenshot --ipc-arg "$out" && printf "saved screenshot to %s\n" "$out"']]
